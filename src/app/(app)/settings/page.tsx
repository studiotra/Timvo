...
export default async function SettingsPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("full_name, business_name, logo_url, phone_number, address, bank_name, bank_account, bank_routing, tax_rate, tax_id, default_currency, invoice_prefix, default_invoice_footer, default_invoice_terms, default_due_days")
    .eq("id", user.id)
    .single();
    ...
    <div className="max-w-2xl space-y-7">
      <section>
        ...
        <p className="mt-1 text-[11px] text-[var(--text-muted)]">
          Email is managed via Supabase Auth.
        </p>
      </section>
